...
import { PlusIcon, PencilIcon, TrashBinIcon, EyeIcon, ShootingStarIcon } from "../icons";
import { Modal } from "../components/ui/modal";
import { questionsService, type QuestionBankResponse, type CreateQuestionBankRequest, type UpdateQuestionBankRequest, type QuestionBankFilterRequest } from "../services/questions.service";
...
export default function Questions() {
  ...
  const loadSubjects = async () => {
    try {
      ...
      if (subjectsList.length > 0) setAiSubjectId(subjectsList[0].subjectId);
    } catch (err) {
  ...
  const loadQuestions = async (page: number = 1, size: number = 10) => {
    try {
      setLoading(true);
      setError(null);
      const filter: QuestionBankFilterRequest = {
        page,
        pageSize: size,
      };
      ...
      if (subjectQuery) filter.tags = subjectQuery;
      if (typeQuery) filter.questionType = typeQuery;
  ...
  return (
    <>
      ...
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" onClick={() => setIsAiOpen(true)} startIcon={<ShootingStarIcon className="h-4 w-4" />}>
              ...
              {loading ? (
                ...
                filteredQuestions.map((question) => (
                  <TableRow key={question.questionId} className="border-t border-gray-100 dark:border-gray-800">
                    ...
                    <TableCell className="px-6 py-4 text-gray-600 dark:text-gray-300">{question.subjectName || question.tags || '-'}</TableCell>
                    <TableCell className="px-6 py-4">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="mt-1 text-gray-900 dark:text-white">{selectedQuestion.subjectName || selectedQuestion.tags || '-'}</p>
                </div>
                <div>
      ...
      <Modal isOpen={isAiOpen} onClose={() => setIsAiOpen(false)} className="max-w-xl p-6">
        <div className="space-y-4">
          <h3 className="text-xl font-semibold">Tạo Câu Hỏi Mới bằng AI</h3>
          {aiError && <div className="text-sm text-red-600">{aiError}</div>}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Mô tả câu hỏi</label>
            <textarea
              value={aiDescription}
              onChange={(e) => setAiDescription(e.target.value)}
              className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 dark:bg-gray-900 dark:border-gray-700"
              rows={4}
              placeholder="Ví dụ: tạo câu hỏi trắc nghiệm về định lý Pythagoras với 4 lựa chọn"
            />
          </div>
          {/* Tự động dùng môn học đầu tiên, loại, độ khó, điểm mặc định */}
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="outline" onClick={() => setIsAiOpen(false)}>Hủy</Button>
            <Button onClick={async () => {
              try {
                setAiError(null);
                setAiLoading(true);
                const suggestion = await questionsService.generateAIQuestion({
                  description: aiDescription,
                  subjectId: aiSubjectId,
                });
                setCreateForm(suggestion);
                setIsAiOpen(false);
                setIsCreateOpen(true);
              } catch (err) {
                const message = err instanceof Error ? err.message : 'Tạo câu hỏi AI thất bại';
                setAiError(message);
              } finally {
                setAiLoading(false);
              }
            }} disabled={aiLoading}>{aiLoading ? 'Đang tạo...' : 'Tạo câu hỏi'}</Button>
          </div>
        </div>
      </Modal>

      {/* Modal chỉnh sửa */}
      <Modal isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} className="max-w-3xl p-6 max-h-[90vh] overflow-y-auto">
        <div className="space-y-4">
