...
export interface ExamListItemDto {
  ...
  teacherId?: number;
  teacherName?: string;
  createdBy?: number; // ID của user tạo bài thi (từ cột CreatedBy trong DB)
  subjectId?: number;
  subjectName?: string;
...
class ExamsService {
  ...
  async getExams(params?: { pageIndex?: number; pageSize?: number; courseId?: number; teacherId?: number; subjectId?: number; }): Promise<PagedResponse<ExamListItemDto>> {
    try {
      ...
      // Map CreatedBy từ backend nếu có
      if (data.items && Array.isArray(data.items)) {
        data.items = data.items.map((item: any) => ({
          ...item,
          createdBy: item.createdBy || item.CreatedBy || item.createdBy || undefined,
          teacherId: item.teacherId || item.TeacherId || item.createdBy || item.CreatedBy || item.teacherId || undefined,
        }));
      }
