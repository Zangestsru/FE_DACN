...
export function useSubmitExam() {
  return useMutation(
    (data: { examId: number; attemptId: number; answers: any[]; timeSpent?: number }) => examService.submitExam(data),
    {
      onError: (error) => {
        console.error('Error submitting exam:', error);
      },
  ...
  const serviceFunction = React.useCallback(
    async () => {
      ...
      const mappedResult = {
        ...
        percentage: result.percentage || result.Percentage || result.score || result.Score || 0,
        score: result.correctAnswers || result.CorrectAnswers || result.score || result.Score || 0,
        maxScore: result.totalQuestions || result.TotalQuestions || result.maxScore || result.MaxScore || 0,
        ...
        correctAnswers: result.correctAnswers || result.CorrectAnswers || 0,
