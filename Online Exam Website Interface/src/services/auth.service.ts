...
class AuthService {
  // Key dùng để broadcast sự kiện logoutAll giữa các tab trên cùng origin
  ...
  async verifyLoginOtp(email: string, otp: string): Promise<AuthResponse> {
    try {
      ...
      // Backend yêu cầu format PascalCase cho các field
  ...
  /**
   * Logout user
   * @param logoutAll - true = đăng xuất khỏi TẤT CẢ thiết bị, false = chỉ thiết bị hiện tại
   */
  async logout(logoutAll: boolean = false): Promise<void> {
    try {
      console.log('🚪 AuthService.logout', { logoutAll });
      ...
      // Nếu là logout toàn bộ thiết bị, broadcast cho các tab khác
      if (logoutAll) {
        try {
