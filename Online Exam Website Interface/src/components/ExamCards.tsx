...
const ExamCard: React.FC<ExamCardProps> = ({ title, subject, provider, time, questionCount, difficulty, image, examId, slug }) => {
  ...
  const handleRegisterClick = () => {
    navigate(`/exam-detail/${slug}/${examId}`);
  };
        ...
        <div className="card-body p-3">
          ...
          {provider && (
            <p className="card-text small text-muted mb-1">
              <i className="bi bi-person me-1"></i> {provider}
            </p>
          )}
...
const ExamCards: React.FC = () => {
  ...
  const mappedExams = exams.map(exam => {
    // Tạo slug từ title tương tự như trong CertificationExams
    const slug = String(exam?.title || 'exam')
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .toLowerCase()
      .replace(/[^a-z0-9\s-]/g, '')
      ...
      <div className="container-fluid px-3 px-md-4">
        ...
        {error && (
          <div className="alert alert-danger" role="alert">
            ...
            {typeof error === 'string' ? error : (error as Error).message || 'Đã có lỗi xảy ra'}
          </div>
        )}
