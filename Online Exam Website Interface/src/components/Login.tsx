...
export const Login: React.FC<LoginProps> = ({
  ...
  const handleLoginSuccess = (payload: { email: string; requiresVerification: boolean }) => {
    ...
    if (payload.requiresVerification) {
      ...
      // Redirect tất cả các role về trang chủ
      navigate('/');
    }
  };
  ...
  const handleOtpVerify = async (otp: string) => {
    try {
      ...
      if (result && result.token) {
        ...
        if (import.meta.env.DEV) console.log(`User ${emailToVerify} logged in successfully.`);
        // Redirect tất cả các role về trang chủ
        navigate('/');
      }
      setVerifying(false);
    } catch (error: any) {
      if (import.meta.env.DEV) console.error('OTP verification failed', error);
      setVerifying(false);
  ...
  const handleResendOtp = async () => {
    try {
      ...
      if (import.meta.env.DEV) console.error('Resend OTP failed', error);
    }
  };
