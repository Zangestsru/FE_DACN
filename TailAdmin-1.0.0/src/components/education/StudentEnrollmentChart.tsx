...
import { dashboardService, EnrollmentDataPoint } from "../../services/dashboard.service";
...
export default function StudentEnrollmentChart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await dashboardService.getEnrollmentData();
        setEnrollmentData(data);
      } catch (error) {
        console.error('Error fetching enrollment data:', error);
        // Fallback to empty array, will use default data in chart
        setEnrollmentData([]);
      } finally {
        setLoading(false);
      }
  ...
  // Prepare chart data from enrollmentData
  const chartCategories = useMemo(() => {
    return enrollmentData.length > 0 
      ? enrollmentData.map(d => d.month)
      : [
  ...
  const series = useMemo(() => {
    return enrollmentData.length > 0 ? [
      {
        name: "Đăng ký mới",
        data: enrollmentData.map(d => d.newRegistrations),
      },
      {
        name: "Học sinh hoạt động",
        data: enrollmentData.map(d => d.activeStudents),
      },
      {
        name: "Hoàn thành khóa học",
        data: enrollmentData.map(d => d.completedCourses),
      },
    ] : [
      {
        name: "Đăng ký mới",
