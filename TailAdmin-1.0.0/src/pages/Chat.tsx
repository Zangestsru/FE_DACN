      ...
      <>
        ...
        <div className="h-[calc(100vh-100px)] flex flex-col p-4 md:p-6 max-w-7xl mx-auto w-full">
          ...
          <Modal isOpen={modelOpen} onClose={() => setModelOpen(false)}>
            <div className="p-6 w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Select AI Model</h3>
                  <p className="text-sm text-gray-500">Choose the model for response generation</p>
                </div>
                <button onClick={() => setModelOpen(false)} className="text-gray-400 hover:text-gray-600">
              ...
              <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-2">
                {/* Gemini Section */}
                <div>
                  <div className="flex items-center gap-2 mb-3 text-sm font-medium text-brand-600 bg-brand-50 dark:bg-brand-900/20 p-2 rounded-lg w-fit">
                     <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                     Google Gemini
                  </div>
                  {!geminiKey && (
                    <div className="mb-3 p-3 bg-orange-50 text-orange-700 text-xs rounded-lg border border-orange-100">
                      ⚠️ Missing Gemini API Key. Set VITE_GEMINI_API_KEY in .env
                    </div>
                  )}
                  <div className="space-y-2">
                    {getAllGeminiModels().map((m) => (
                      <label key={m} className={`cursor-pointer group flex items-center justify-between p-3 rounded-xl border transition-all ${selectedModel === m && selectedProvider === 'gemini' ? 'border-brand-500 bg-brand-50 dark:bg-brand-900/20 ring-1 ring-brand-500' : 'border-gray-200 dark:border-gray-700 hover:border-brand-300 dark:hover:border-gray-600'}`}>
                        <div className="flex items-center gap-3">
                          <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${selectedModel === m && selectedProvider === 'gemini' ? 'border-brand-500' : 'border-gray-400'}`}>
                            {selectedModel === m && selectedProvider === 'gemini' && <div className="w-2 h-2 rounded-full bg-brand-500" />}
                          </div>
                          <span className="text-sm font-medium text-gray-700 dark:text-gray-200">{m}</span>
                        </div>
                        {selectedModel === m && selectedProvider === 'gemini' && <span className="text-xs font-bold text-brand-600">Active</span>}
                      </label>
                    ))}
                  </div>
                </div>
                ...
                {/* Open Source Section */}
                <div>
                  <div className="flex items-center gap-2 mb-3 text-sm font-medium text-purple-600 bg-purple-50 dark:bg-purple-900/20 p-2 rounded-lg w-fit">
                  ...
                  {!ossKey && (
                    <div className="mb-3 p-3 bg-orange-50 text-orange-700 text-xs rounded-lg border border-orange-100">
                      ⚠️ Missing Groq API Key. Set VITE_GROQ_API_KEY in .env
                    </div>
                  )}
                  <div className="space-y-2">
                    {getAllOssModels().map((m) => (
                      <label key={m} className={`cursor-pointer group flex items-center justify-between p-3 rounded-xl border transition-all ${selectedModel === m && selectedProvider === 'groq' ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/20 ring-1 ring-purple-500' : 'border-gray-200 dark:border-gray-700 hover:border-purple-300 dark:hover:border-gray-600'}`}>
                        <div className="flex items-center gap-3">
                          <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${selectedModel === m && selectedProvider === 'groq' ? 'border-purple-500' : 'border-gray-400'}`}>
                            {selectedModel === m && selectedProvider === 'groq' && <div className="w-2 h-2 rounded-full bg-purple-500" />}
                          </div>
                          <span className="text-sm font-medium text-gray-700 dark:text-gray-200">{m}</span>
                        </div>
                        {selectedModel === m && selectedProvider === 'groq' && <span className="text-xs font-bold text-purple-600">Active</span>}
                      </label>
                    ))}
                  </div>
                </div>
                ...
                <Button onClick={() => { 
                  try { 
                    ...
                    if (selectedProvider === 'gemini') { 
                      ...
                      const set2 = Array.from(new Set([current2, selectedModel].join(',').split(',').map(s => s.trim()).filter(Boolean))); 
