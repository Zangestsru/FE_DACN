...
export default function Courses() {
  ...
  const loadDropdowns = async () => {
    try {
      ...
      const [subjectsData, teachersData] = await Promise.all([
        ...
        usersService.getUsers({ role: 'teacher' }),
      ]);
      setSubjects(subjectsData);
      setTeachers(teachersData.users.filter(u => u.role?.toLowerCase() === 'teacher'));
    } catch (err) {
  ...
  const handleCreateCourse = async () => {
    ...
    try {
      ...
      await coursesService.createCourse(createForm);
      await loadCourses();
  ...
  const handleAddMaterial = async () => {
    ...
    try {
      ...
      // Reload materials if detail modal is open for the same course
      if (isDetailOpen && selectedCourse && selectedCourse.courseId) {
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    // Find teacher by comparing string id with number teacherId
                    const selectedTeacher = teachers.find(t => Number(t.id) === teacherId);
                    console.log('📝 Selected teacher ID:', teacherId, 'Teacher:', selectedTeacher, 'All teachers:', teachers.map(t => ({ id: t.id, name: t.fullName })));
                    setCreateForm({ ...createForm, teacherId });
                  }}
                  className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 dark:bg-gray-900 dark:border-gray-700"
                >
                  <option value="">Chọn giáo viên</option>
                  {teachers.map((t) => (
                    <option key={t.id} value={t.id}>{t.fullName} (ID: {t.id})</option>
                  ))}
                  {teachers.length === 0 && (
            ...
            <div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditForm({ ...editForm, teacherId: e.target.value ? Number(e.target.value) : undefined })}
                  className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 dark:bg-gray-900 dark:border-gray-700"
                >
                  <option value="">Chọn giáo viên</option>
                  {teachers.map((t) => (
