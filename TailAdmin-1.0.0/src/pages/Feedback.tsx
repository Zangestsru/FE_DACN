...
import { EyeIcon, TrashBinIcon, ArrowUpIcon as RefreshIcon } from "../icons";
...
export default function FeedbackPage() {
  ...
  const loadCourseFeedbacks = async () => {
    try {
      ...
      setError(null);
      const data = await courseFeedbackService.getAllCourseFeedbacks();
      setCourseFeedbacks(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Lỗi khi tải đánh giá khóa học');
  ...
  const loadExamFeedbacks = async () => {
    try {
      setExamLoading(true);
      setError(null);
      const data = await examFeedbackService.getAllExamFeedbacks();
      setExamFeedbacks(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Lỗi khi tải đánh giá bài thi');
  ...
  const handleDeleteCourseFeedback = async (feedbackId: number) => {
    if (!confirm('Bạn có chắc chắn muốn xóa đánh giá này?')) return;

    try {
      await courseFeedbackService.deleteCourseFeedback(feedbackId);
  ...
  const handleDeleteExamFeedback = async (feedbackId: number) => {
    if (!confirm('Bạn có chắc chắn muốn xóa đánh giá này?')) return;

    try {
      await examFeedbackService.deleteExamFeedback(feedbackId);
      await loadExamFeedbacks();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Lỗi khi xóa đánh giá');
    }
  };
  ...
  const filteredExamFeedbacks = useMemo(() => {
    ...
    return filtered;
  }, [examFeedbacks, examSearchQuery, examRatingFilter]);
  ...
  const filteredCourseFeedbacks = useMemo(() => {
    ...
    return filtered;
  }, [courseFeedbacks, courseSearchQuery, courseRatingFilter]);

  return (
    <>
      <PageMeta title="Quản Lý Phản Hồi" description="Xem và quản lý phản hồi của bài thi" />
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
            <Button
              ...
              disabled={activeTab === "course" ? courseLoading : examLoading}
            >
              Làm mới
            </Button>
          </div>
                      ...
                      <TableCell className="px-6 py-4">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <Button 
                            ...
                            onClick={() => handleDeleteExamFeedback(feedback.feedbackId)}
                            title="Xóa"
                          >
                      ...
                      <TableCell className="px-6 py-4">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <Button 
                            size="sm" 
                            className="!bg-red-500 hover:!bg-red-600 !p-2" 
                            onClick={() => handleDeleteCourseFeedback(feedback.feedbackId)}
                            title="Xóa"
                          >
                            <TrashBinIcon className="h-4 w-4 fill-white" />
                          </Button>
        ...
        {selectedCourseFeedback && (
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <Button 
                ...
                onClick={() => {
                  if (confirm('Bạn có chắc chắn muốn xóa đánh giá này?')) {
                    handleDeleteCourseFeedback(selectedCourseFeedback.feedbackId);
              ...
              <Button variant="outline" onClick={() => setIsCourseViewOpen(false)}>Đóng</Button>
            </div>
          </div>
        )}
      </Modal>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <Button 
                ...
                onClick={() => {
                  if (confirm('Bạn có chắc chắn muốn xóa đánh giá này?')) {
                    handleDeleteExamFeedback(selectedExamFeedback.feedbackId);
                    setIsExamViewOpen(false);
                  }
                }}
              >
                Xóa đánh giá
