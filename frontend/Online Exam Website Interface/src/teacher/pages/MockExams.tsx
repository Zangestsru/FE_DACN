...
export default function MockExams() {
  const initialExams = useMemo<MockExam[]>(
    () => [
      {
        id: "m1",
      ...
      {
        id: "m2",
        name: "Thi Thử THPT Quốc Gia Văn",
        subject: "Văn",
        date: "2025-12-22",
        duration: 120,
        totalMarks: 10,
  ...
  const [list, setList] = useState<MockExam[]>(initialExams);
  const [selected, setSelected] = useState<MockExam | null>(null);
  const [isEditOpen, setIsEditOpen] = useState(false);
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const [editForm, setEditForm] = useState<MockExam>({
    id: "",
    name: "",
    subject: "",
    date: "",
  ...
  const [createForm, setCreateForm] = useState<MockExam>({
    id: "",
    name: "",
    subject: "",
  ...
  useEffect(() => { if (!alert) return; const t = setTimeout(() => setAlert(null), 3000); return () => clearTimeout(t); }, [alert]);
  ...
  const handleUpdate = () => {
    ...
    setIsEditOpen(false);
    showAlert("success", "Lưu thành công", "Bài thi thử đã được cập nhật.");
  };

  const openDelete = (exam: MockExam) => {
    setSelected(exam);
  ...
  const confirmDelete = () => {
    ...
    showAlert("success", "Đã xóa", "Bài thi thử đã được xóa khỏi danh sách.");
  };

  const validateExam = (e: MockExam): string | null => {
    if (!e.name.trim()) return "Vui lòng nhập tên bài thi thử";
  ...
  const handleCreate = () => {
    ...
    const newExam: MockExam = { ...createForm, id: `m${Date.now()}` };
    setList((prev) => [newExam, ...prev]);
    setIsCreateOpen(false);
    showAlert("success", "Tạo thành công", "Bài thi thử mới đã được thêm vào danh sách.");
  };

  return (
    <>
      <PageMeta title="Quản Lý Bài Thi Thử" />
      <div className="p-6">
        {alert && (
          <div className="mb-4">
        ...
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-semibold">Quản Lý Bài Thi Thử</h1>
          <Button onClick={() => { setCreateForm({ id: "", name: "", subject: "", date: new Date().toISOString().split("T")[0], duration: 90, totalMarks: 10, questionsCount: 50, status: "draft" }); setIsCreateOpen(true); }}>Tạo Bài Thi Thử Mới</Button>
        </div>
        ...
        {/* Modal sửa bài thi thử */}
        <Modal isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} className="max-w-xl p-6">
          <div className="space-y-4">
            <h3 className="text-xl font-semibold">Sửa Bài Thi Thử</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-sm text-gray-600">Tên bài thi thử</label>
                <input
                  value={editForm.name}
        ...
        <Modal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} className="max-w-xl p-6">
          <div className="space-y-4">
            <h3 className="text-xl font-semibold">Tạo Bài Thi Thử Mới</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-sm text-gray-600">Tên bài thi thử</label>
                <input
